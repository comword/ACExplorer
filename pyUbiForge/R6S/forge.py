...
class Forge(BaseForge):
    ...
    def __init__(self, path: str, forge_file_name: str):
        ...
        forge_file_version, file_data_header_offset = forge_file.read_struct(
            'iQ')
        if forge_file_version > 30:
            raise Exception(
                f'Unsupported Forge file format : "{forge_file_version}"')
        forge_file.read_uint_32()  # gloablMetaFileKey
        forge_file.read_uint_32()  # x19
        forge_file.read_uint_8()  # x1d: 0x0

        numEntries = forge_file.read_uint_32()
        numDirectories = forge_file.read_uint_32()
        logging.debug(
            f"numEntries:{numEntries}, numDirectories:{numDirectories}")
        forge_file.read_uint_32()  # unk2 0x22
        forge_file.read_uint_32()  # unk3 0x26
        if forge_file_version >= 27:
            forge_file.read_uint_32()  # unk3b 0x2a

        forge_file.read_uint_32()  # firstFreeFile 0x2e
        forge_file.read_uint_32()  # firstFreeDir 0x32
        sizeOfFat = forge_file.read_uint_32()  # 0x36
        numTables = forge_file.read_uint_32()  # 0x3a
        logging.debug(f"sizeOfFat:{sizeOfFat}, numTables:{numTables}")
        file_data_offset = forge_file.read_int_64()  # 0x3e
        forge_file.seek(file_data_offset)
        # File Data at 0x4a
        for i in range(numTables):
            self._entries.append(EntryTable(forge_file, forge_file_version))

        for i in range(len(self._entries)):
            entry: EntryTable = self._entries[i]
            for n in range(entry.index_count):
                if entry.index_table['raw_data_size'][n] != entry.name_table['raw_data_size'][n]:
                    raise Exception(
                        'These should be the same. Is something wrong?')
                self._datafiles[int(entry.index_table['file_id'][n])] = DataFile(
                    int(entry.index_table['raw_data_offset'][n]),
                    int(entry.index_table['raw_data_size'][n]),
                    entry.name_table['file_name'][n].replace(
                        b'\x00', b'').decode("utf-8"),
                    int(entry.name_table['file_type'][n])
                )
